...
public class FlagFilter extends LineFilter {
  ...
  /*
    ...
    OPT_STRICT_REMOVE_FILTERED_ALL  to pass, all listed flag must not be oresent, and no other (sites kept by OPT_STRICT_KEEP_FILTERED_ALL are removed and vice versa)
    OPT_STRICT_REMOVE_FILTERED_ANY  to pass, all listed flag must not be preent (sites kept by OPT_STRICT_KEEP_FILTERED_ANY are removed and vice versa)
  */
  
  /**
   * 
   * @param keep
   * @param matchesAll true -> keep/reject only when all flag are in the list; false -> keep/recjt when at least one flag is in the list

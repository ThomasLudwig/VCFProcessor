...
public abstract class Filter<T> {
   ...
   * <p>
   * We read a new raw_line, and split it into a Strnig Array : read_line[]
   * <p>
   * #1 First, apply the "Quick" filters, that are independant of the number of samples (they are based on chr,pos,id,ref,alt...)
   * #2 Remove Samples (if needed), so the filtered_line[] has less columns than the read_line[] //this is fast, but it useless to do it on le lines filters in
   * #1
   ...
   * filtered
   * implicitely create a new NonVariant filter and add it, it will be executed in 4
   * <p>
   ...
   * - after is more efficient, since we process less lines
